...
my ($Global,$Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub, $Reqsub2,$Resource,$Job_prefix,$Verbose, $Help, $getmem);
GetOptions(
	"global"=>\$Global,
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
...
while (@Shell) {
	...
	my $job_cmd = "qsub -S /bin/sh ";  ## -l h_vmem=16G,s_core=8
	...
	for (my $i=0; $i<@Shell; $i++) {
		while (1) {
			...
			if ( $run_num != -1 && ($i < $Maxjob || $run_num < $Maxjob) ) {#������������������������run_num==-1��i < Maxjob��������������������������������������������������������
				my $jod_return = `$job_cmd -l $part_resource $Shell[$i]`;
				sleep(2);#20160219 ��������������������������������������2������
				if($jod_return =~ /(\d+\.mgmt\.hpc)/){
					#$job_id = $1;
					my @job_idparts = split /\./,$1;
					$job_id = "$job_idparts[0].$job_idparts[1]";
...
sub run_count {
	...
	#my $user = $whoami;
	my $qstat_result = `qstat 2>\&1`;
	#$user = substr($user, 0, 12);
	if ($qstat_result =~ /failed receiving gdi request/) {
		$run_num = -1;
		return $run_num; ##����������
	}
    if ($qstat_result =~ /error/){
        $run_num = -1;
        return $run_num; # �������������������� 20170419
    }
    
	my @jobs = split /\n/,$qstat_result;
	my %died;
	died_nodes(\%died) if (@jobs > 0); ##the compute node is down, ����������������������������������
	foreach my $job_line (@jobs) {
		$job_line =~s/^\s+//;
		my @job_field = split /\s+/,$job_line;
	        next if($job_field[4] eq 'C' || $job_field[4] eq 'Time' || $job_field[4] eq '-');
#		next if($job_field[3] ne $user);
		if (exists $all_p->{$job_field[0]}){
			my $node_name = $1 if($job_field[7] =~ /(compute-\d+-\d+)/);
			#if ( !exists $died{$node_name} && ($job_field[4] eq "H" || $job_field[4] eq "Q" || $job_field[4] eq "R" || $job_field[4] eq "T" || $job_field[4] eq "W" ||  $job_field[4] eq "E" || $job_field[4] eq "S") ) {
			if ( $job_field[4] eq "H" || $job_field[4] eq "Q" || $job_field[4] eq "R" || $job_field[4] eq "T" || $job_field[4] eq "W" ||  $job_field[4] eq "E" || $job_field[4] eq "S" ) {
				$run_p->{$job_field[0]} = $job_field[1]; ##job id => shell file name
				$run_num++;
				print "Hi";
				if ((defined $getmem) && ($job_field[4] eq "R")){### get mem ;  add by nixiaoming nixiaoming@genomics.cn
					my $jobinfo=`qstat -j $job_field[0] 2>&1 |grep -i resource`;
					$$memlist{$all_p->{$job_field[0]}}=$jobinfo;
				}
			}else{
				print STDERR "qdel $job_field[0] for job run in died node $node_name\n";#�������������� 20160830
				`qdel $job_field[0]`;
			}
		}
	}
